...
getNewQuestion = () => {
    ...
    if (availableQuesions.length === 0 || questionCounter >= MAX_QUESTIONS) {
        ...
        // Genera il PDF e attendi il completamento prima di reindirizzare
        generatePDF().then(() => {
            setTimeout(() => {
                window.location.assign('/quizprova/end.html');
            }, 2000); // Ritardo di 2 secondi per consentire il download
        });
...
function generatePDF() {
    return new Promise((resolve) => {
        ...
        // Risolvi la Promise dopo il download
        resolve();
    });
}
